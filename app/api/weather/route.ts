...
async function getDailyUV(lat: number, lon: number): Promise<Record<string, number>> {
  // Open-Meteo returns daily UV index max — no API key needed.
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=uv_index_max&forecast_days=6&timezone=UTC`;
    const r = await fetch(url, { headers: { "Accept": "application/json" } });
    if (!r.ok) return {};
    const j: any = await r.json();
    const dates: string[] = j?.daily?.time ?? [];
    const uvs: number[] = j?.daily?.uv_index_max ?? [];
    const map: Record<string, number> = {};
    for (let i = 0; i < dates.length; i++) {
      if (dates[i] && Number.isFinite(uvs[i])) {
        map[dates[i]] = Math.round(uvs[i] * 10) / 10;
      }
    }
    return map;
  } catch {
...
export async function GET(req: Request) {
  ...
  // Fetch UV index from OpenWeather (free endpoint)
  ...
  try {
    ...
    if (uvRes.ok) {
      ...
      currentUV = Number(uvData?.value ?? null);
    }
  } catch {
    // UV fetch failed, continue without it
  }
  ...
  const blocks: ForecastBlock[] = (forecast.list ?? []).slice(0, 40).map((b: any) => {
    ...
    return {
      ...
      precipMm,
      pop: Math.round((b.pop ?? 0) * 100), // OWM gives 0–1, convert to 0–100%
      humidity: b.main.humidity ?? null,
  ...
  const [{ past24, past48 }, dailyUVMap] = await Promise.all([
    getPastPrecipMm(Number(lat), Number(lon)),
    getDailyUV(Number(lat), Number(lon)),
  ]);
  ...
  const daily = dayKeys.map((key) => {
    ...
    const popMax = Math.max(...dayBlocks.map((b) => b.pop ?? 0)); // highest chance of rain across all blocks
    const humidityAvg = Math.round(
      dayBlocks.reduce((sum, b) => sum + (b.humidity ?? 0), 0) / dayBlocks.length
    );
    ...
    return {
      ...
      pop: Number.isFinite(popMax) ? popMax : null,
